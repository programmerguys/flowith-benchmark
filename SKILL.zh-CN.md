...
description: 公开 benchmark 协议技能中文译本。
---
...
- 这个仓库是协议与提交入口仓库，不是官方规定的 benchmark runner 包。
- 不要仅仅因为仓库名叫 `flowith-benchmark`，就假设一定存在一个可安装的 `flowith-benchmark` CLI 或可执行程序。
- 除非你明确需要这里的文档、schema 或 issue template，否则不要把这个仓库当作 benchmark runtime 去 clone / install。
- 如果你的环境已经提供 benchmark runner、任务执行器或内部编排工具，可以直接使用；并把真实使用的 runner 名称和环境信息记录进 `run_meta.json`。
...
执行工具策略：
- 本 skill 故意不绑定具体执行引擎。
- 你可以使用任何适合当前环境的 runner、编排层或执行工作流。
- 本 skill 只定义 benchmark 协议和证据契约，不规定包名、安装步骤或必须采用的执行接口。
...
1. **Task pack（数据集）：**  
   **默认：AgentIF-OneDay**  
   ...
   官方入口：
   - 数据集主页：`https://huggingface.co/datasets/xbench/AgentIF-OneDay`
   - 数据集 README 直链：`https://huggingface.co/datasets/xbench/AgentIF-OneDay/resolve/main/README.md`
   - 任务定义直链：`https://huggingface.co/datasets/xbench/AgentIF-OneDay/resolve/main/data.jsonl`
   - 附件目录：`https://huggingface.co/datasets/xbench/AgentIF-OneDay/tree/main/Attachments`
   - 官方仓库入口：`https://huggingface.co/datasets/xbench/AgentIF-OneDay/tree/main`

   拉取策略：
   - 你可以自由选择拉取方法和工具，但必须从官方公开信源拉到同一份 benchmark 文件。
   - 当存在公开文件直链时，优先直接使用这些直链。
   - GitHub mirror 只作为兜底或人工阅读文档入口，不作为 canonical source of record。

   推荐的本地目录结构：
   - Workspace root：`<你的工作目录>/os-benchmark/`
   - Dataset root：`<你的工作目录>/os-benchmark/dataset/AgentIF-OneDay/`
   - Run root：`<你的工作目录>/os-benchmark/runs/<run_id>/`

   预期文件：
   - `data.jsonl`（任务定义）
   - `README.md`（数据集说明）
   - `Attachments/`（任务附件）
   - 可选：`task-filter.json`（如果该 variant 提供 track filter）
   - 可选：`VERSION`（如果该 variant 提供数据集版本号）

   执行范围规则：
   - 如果没有 `VERSION`，就用信源 commit hash 或不可变的数据集快照标识作为 `dataset_version`。
...
在进入正式执行前，先确认 dataset root 中已经存在预期文件。如果必需文件缺失，不要带着残缺数据启动 benchmark，先把下载问题修好。
